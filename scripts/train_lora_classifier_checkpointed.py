...
Aggressive RTX 4060 test run:
    python scripts/train_lora_classifier.py `
      --output-dir models/v3_lora_classifier_r32_alpha64_len384_batch64_expD `
      --epochs 1 `
      --batch-size 64 `
      --eval-batch-size 32 `
      --grad-accum-steps 1 `
...
Resume:
    python scripts/train_lora_classifier.py `
      --resume-from models/v3_lora_classifier_r32_alpha64_len384_batch64_expD/checkpoint_last.pt `
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["query", "value"],
        help="Module name suffixes to LoRA-adapt. For BERT/SciBERT, query value is a safe start.",
    )

    parser.add_argument("--eval-every", type=int, default=3000, help="Evaluate every N optimizer steps.")
    parser.add_argument("--max-val-samples", type=int, default=30000, help="0 means full validation set.")
    parser.add_argument("--patience", type=int, default=4, help="Early stop after N evals without macro-F1 improvement.")
    parser.add_argument(
        "--thresholds",
        type=str,
    ...
    log("Starting Day 7 LoRA fine-tuning")
    ...
    log(f"pos_weight range: min={pos_weights_np.min():.1f}, max={pos_weights_np.max():.1f}, median={np.median(pos_weights_np):.1f}")
